...
<template v-disable-right-click>
  <v-app>
        ...
        <v-row>
          <v-col cols="12" class="pa-5 ma-0">
            <div style="height: 82vh;">
              <Nuxt v-disable-right-click class="nuxt-custom-height" />
            </div>
          </v-col>
        </v-row>
        <v-divider />
      ...
      <v-card>
        ...
        <v-card-actions align="space-between">
          ...
          <v-btn
            ...
            class="white--text"
            @click="cancelOrder = false, goToHome()"
          >
        ...
        <v-row class="pa-10 mt-5">
          <v-col align="center">
            <v-btn icon @click="commandType('place')">
              <v-img
                :src="require('../assets/imgs/sur_place.png')"
                height="100"
                width="100"
              />
            </v-btn>
          ...
          <v-col align="center">
            <v-btn icon @click="commandType('emporter')">
              <v-img
                :src="require('../assets/imgs/carry.png')"
                height="100"
                width="100"
              />
            </v-btn>
  ...
  methods: {
    ...
    async getDataCategorie () {
      ...
      const response = await this.$axios({
        method: 'get',
        url: `/api/categorie/read/all/restaurant/${this.restaurantId}`
      })
    ...
    async validCommand (index) {
      ...
      this.get()('card').forEach((item) => {
        command = command ? command + `,${item.name}` : item.name
      })
      const { data } = await this.$axios({
        ...
        data: {
          ...
          order: command,
          totalPrive: this.get()('card').reduce((accumulator, object) => {
            return accumulator + object.price
          }, 0).toFixed(2),
          code: factCode
        }
